...
var camera,map,mapData=[],attributes=[],textures=[],samples=[],interacts=[void 0,interact_Door,interact_Panel,interact_Secret,interact_Item,interact_End,interact_Audio],keys=[],keysRepeat=[],gameTime="00:00:00",gameStarted=!1,gameCompleted=!1,framesRender=0,frameTime=0,inMobile=navigator.userAgent.match(/Android/i),width=2*NUM_RAYS+30,height=600,ctx,floorGradient,ceilingGradient;function DBG(a,b){(b&&b<=DBGLEVEL||3==DBGLEVEL)&&console.log(a)}
...
a)return b}gotoBlock(a){a=this.findBlock(a);camera.move(a.center.x-BLOCK_SIZE,a.center.y)}doCull(a){var b=[];this.nearest=[];for(var c of this.blocks)c.distance=Math.pow(c.center.x-a.po.x,2)+Math.pow(c.center.y-a.po.y,2),c.distance<NEAR_MARGIN&&this.nearest.push(c),a.cull(c)&&b.push(c);return b}sortBlocks(a){a.sort(function(a,c){return a.distance-c.distance})}drawLine(a,b,c){ctx.strokeStyle=0==c?"#444444":"#ffffff";ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke()}drawBlocks(a){for(a of this.blocks){var b=
project({x:a.l1.x,y:a.l3.y}),c=project({x:a.l2.x,y:a.l3.y}),e=project({x:a.l2.x,y:a.l4.y}),d=project({x:a.l1.x,y:a.l4.y});this.drawLine(b,c,a.l3.ray);this.drawLine(c,e,a.l2.ray);this.drawLine(e,d,a.l4.ray);this.drawLine(d,b,a.l1.ray)}}draw(a,b,c){this.visible=this.doCull(a);this.sortBlocks(this.visible);b&&(this.drawBlocks(),a.draw());if(c){a.initRays();for(b=a.getRay();b;){c=this.intersect(b,CAST_MAX_DEPTH);for(var e of c){c=projectToLine(b.rayIndex,e.d);var d=e.x-e.cx,g=e.y-e.cy;d=Math.floor(TEXTURE_SIZE/
BLOCK_SIZE*Math.sqrt(d*d+g*g));g=textures[e.block.attr.texture];g.mask&&(ctx.globalAlpha=1,ctx.drawImage(g.mask,d,0,1,TEXTURE_SIZE,c.p0.x,c.p0.y,2,c.p1.y-c.p0.y));ctx.globalAlpha=c.alpha;ctx.drawImage(g.tx,d,0,1,TEXTURE_SIZE,c.p0.x,c.p0.y,2,c.p1.y-c.p0.y)}b=a.getRay()}ctx.globalAlpha=1}}}
...
this.yadd;this.rayIndex++;return a}}draw(){var a=project(this.pl),b=project(this.pr),c=project(this.po);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.lineTo(c.x,c.y);ctx.lineTo(a.x,a.y);ctx.stroke()}}
